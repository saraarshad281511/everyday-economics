/**
 * Fills an EMPTY database with sample sections, authors, articles and pages.
 * Run with:  pnpm seed
 * Safe to run once; it stops if articles already exist.
 */
...
async function run() {
  ...
  if (existing.totalDocs > 0) {
    payload.logger.info('Articles already exist – seed skipped.')
    process.exit(0)
  }
  ...
  const adminExists = await payload.count({ collection: 'users', where: { email: { equals: adminEmail } } })
  if (!adminExists.totalDocs) {
  ...
  payload.logger.info('Creating sections…')
  ...
  for (const [i, s] of SECTIONS.entries()) {
    ...
    sectionIds[s.slug] = c.id
  }

  payload.logger.info('Creating authors…')
  ...
  payload.logger.info(`Done! Log in at /admin with ${adminEmail} / ${adminPassword} and change the password.`)
  process.exit(0)
}
